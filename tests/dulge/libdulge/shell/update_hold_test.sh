...
update_pkg_with_held_dep_head() {
	atf_set "descr" "dulge-install(1): update packages with held dependency (issue #143)"
}
...
update_pkg_with_held_dep_body() {
	...
	dulge-rindex -d -a pkgheld*.dulge
	atf_check_equal $? 0

	dulge-install -r root -C empty.conf --repository=$PWD -y pkgheld
	atf_check_equal $? 0
	dulge-pkgdb -r root -m hold pkgheld

	dulge-rindex -d -a pkginst*.dulge
	atf_check_equal $? 0

	dulge-rindex -d -a pkgdep-22*.dulge
	atf_check_equal $? 0

	dulge-install -r root -C empty.conf --repository=$PWD -d -yv pkginst
	atf_check_equal $? 19
}
...
hold_update_revdep_body() {
	mkdir -p repo empty
	cd repo
	dulge-create -A noarch -n pari-2.11.4_1 -s "pari pkg" ../empty
	dulge-create -A noarch -n pari-devel-2.11.4_1 --dependencies="pari>=2.11.4_1" -s "pari-devel pkg" ../empty
	atf_check_equal $? 0
	dulge-rindex -d -a $PWD/*.dulge
	atf_check_equal $? 0
	cd ..

	dulge-install -r root --repository=$PWD/repo -yd pari pari-devel
	atf_check_equal $? 0

	dulge-pkgdb -r root -m hold pari
	atf_check_equal $? 0

	cd repo
	dulge-create -A noarch -n pari-devel-2.13.1_1 --dependencies="pari>=2.13.1_1" -s "pari-devel pkg" ../empty
	atf_check_equal $? 0
	dulge-rindex -d -a $PWD/*.dulge
	atf_check_equal $? 0
	cd ..

	dulge-install -r root --repository=$PWD/repo -dvyu
	atf_check_equal $? 19
}
...
update_hold_update_revdep_body() {
	...
	cd repo
	dulge-create -A noarch -n pari-2.13.1_1 -s "pari pkg" ../empty
	dulge-create -A noarch -n pari-devel-2.13.1_1 --dependencies="pari>=2.13.1_1" -s "pari-devel pkg" ../empty
	atf_check_equal $? 0
	dulge-rindex -d -a $PWD/*.dulge
	atf_check_equal $? 0
...
hold_install_revdep_body() {
	...
	dulge-install -r root --repository=$PWD/repo -yd pari
	atf_check_equal $? 0
	...
	dulge-install -r root --repository=$PWD/repo -dvy pari-devel
	atf_check_equal $? 19
}
