...
tc1_body() {
	...
	echo "noextract=/usr/bin/*" > root/dulge.d/foo.conf
	...
	dulge-pkgdb -C dulge.d -r root A
	atf_check_equal $? 0
}
...
tc2_body() {
	...
	mkdir -p root/dulge.d
	echo "noextract=/usr/bin/f*" > root/dulge.d/foo.conf
...
tc3_body() {
	...
	mkdir -p root/dulge.d
	echo "noextract=/usr/bin/*" > root/dulge.d/foo.conf
	echo "noextract=!/usr/bin/blah" >> root/dulge.d/foo.conf
...
tc4_body() {
	...
	echo "noextract=/usr/bin/*" > root/dulge.d/foo.conf
	echo "noextract=!/usr/bin/blah" >> root/dulge.d/foo.conf
	echo "noextract=/usr/bin/bla*" >> root/dulge.d/foo.conf
...
tc5_body() {
	...
	mkdir -p root/dulge.d
	echo "noextract=*foo" > root/dulge.d/foo.conf
