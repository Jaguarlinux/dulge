...
tc1_body() {
	...
	dulge-create -A noarch -n a-0.1_1 -s "pkg a" --config-files "/cf1.conf" pkg_a
	atf_check_equal $? 0
	rm -rf pkg_a
	...
	mkdir pkg_a
	echo "fooblah" > pkg_a/cf1.conf
	dulge-create -A noarch -n a-0.2_1 -s "pkg a" --config-files "/cf1.conf" pkg_a
	atf_check_equal $? 0
	dulge-rindex -d -a $PWD/*.dulge
	rm -rf pkg_a
	atf_check_equal $? 0
...
tc4_body() {
	...
	dulge-create -A noarch -n a-0.1_1 -s "pkg a" --config-files "/etc/cf1.conf" pkg_a
	atf_check_equal $? 0
	rm -rf pkg_a
	dulge-rindex -d -a $PWD/*.dulge
	atf_check_equal $? 0
...
tc6_body() {
	...
	mkdir -p rootdir/dulge.d
	echo "keepconf=true" > rootdir/dulge.d/keepconf.conf

	dulge-install -C dulge.d -r rootdir --repository=$PWD/repo -yvd a
	atf_check_equal $? 0
	...
	dulge-install -C dulge.d -r rootdir --repository=$PWD/repo -yuvd
	atf_check_equal $? 0
	result="$(cat rootdir/cf1.conf)"
	rval=1
...
tc11_body() {
	...
	cd repo
	atf_check -o ignore -- dulge-create -A noarch -n a-0.1_1 -s "pkg a" --config-files "/cf1.conf" ../pkg_a
	atf_check -o ignore -- dulge-rindex -a $PWD/*.dulge
	cd ..

	atf_check -o ignore -- dulge-install -C dulge.d -r rootdir --repository=$PWD/repo -yv a

	echo "userdata" >rootdir/cf1.conf

	cd repo
	echo "backup" > ../pkg_a/cf1.conf
	atf_check -o ignore -- dulge-create -A noarch -n a-0.2_1 -s "pkg a" --config-files "/cf1.conf" ../pkg_a
	atf_check -o ignore -e ignore -- dulge-rindex -a $PWD/*.dulge
	cd ..

	atf_check -o ignore -- dulge-install -C dulge.d -r rootdir --repository=$PWD/repo -yuv
