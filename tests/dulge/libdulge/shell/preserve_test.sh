...
preserve_update_body() {
	...
	cd some_repo
	dulge-create -A noarch -n A-1.0_1 -s "A pkg" --preserve ../pkg_A
	atf_check_equal $? 0
	dulge-rindex -d -a $PWD/*.dulge
	atf_check_equal $? 0
	cd ..

	dulge-install -r root --repository=$PWD/some_repo -yd A
	atf_check_equal $? 0

	cd some_repo
	mv ../pkg_A/1.0 ../pkg_A/1.1
	rm -f *.dulge
	dulge-create -A noarch -n A-1.1_1 -s "A pkg" --preserve ../pkg_A
	atf_check_equal $? 0
	dulge-rindex -d -a $PWD/*.dulge
	atf_check_equal $? 0
	cd ..
...
preserve_update_conflict_body() {
	...
	cd some_repo
	rm -f *.dulge
	dulge-create -A noarch -n A-1.0_2 -s "A pkg" --preserve ../pkg_A
	atf_check_equal $? 0
	dulge-rindex -d -a $PWD/*.dulge
	atf_check_equal $? 0
	cd ..
