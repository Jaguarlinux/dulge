...
vpkg_dont_update_body() {
	...
	cd some_repo
	dulge-create -A noarch -n A-1.0_1 -s "A pkg" --provides "libEGL-1.0_1" ../pkg_A
	atf_check_equal $? 0
	dulge-create -A noarch -n B-1.0_1 -s "B pkg" --dependencies "A>=0" ../pkg_B
	atf_check_equal $? 0
	dulge-create -A noarch -n C-1.0_1 -s "C pkg" --dependencies "libEGL>=2.0" ../pkg_C
	atf_check_equal $? 0
	dulge-create -A noarch -n D-1.0_1 -s "D pkg" --provides "libEGL-2.0_1" ../pkg_D
	atf_check_equal $? 0
	...
	out=$(dulge-query  -r root -l|awk '{print $2}'|tr -d '\n')
...
vpkg_replace_provider_body() {
	...
	dulge-create -A noarch -n B-1.0_1 -s "B pkg" --dependencies "A>=0" ../pkg_B
	atf_check_equal $? 0
	dulge-create -A noarch -n C-1.0_1 -s "C pkg" --provides "A-1.0_1" --replaces="A>=0" ../pkg_C
	atf_check_equal $? 0
	dulge-create -A noarch -n D-1.0_1 -s "D pkg" --dependencies "C>=0" ../pkg_D
	atf_check_equal $? 0
	...
	dulge-install  -r root --repository=$PWD/some_repo -dy B
	atf_check_equal $? 0
	dulge-install  -r root --repository=$PWD/some_repo -dy D
	atf_check_equal $? 0
...
vpkg_provider_in_trans_body() {
	...
	cd some_repo
	dulge-create -A noarch -n gawk-1.0_1 -s "gawk pkg" ../pkg_gawk
	atf_check_equal $? 0
	dulge-create -A noarch -n base-system-1.0_1 -s "base-system pkg" --dependencies "gawk>=0" ../pkg_base-system
	atf_check_equal $? 0
	...
	dulge-install  -r root --repository=$PWD/some_repo -dy base-system
	atf_check_equal $? 0

	cd some_repo

	dulge-create -A noarch -n gawk-1.1_1 -s "gawk pkg" --provides "awk-1.0_1" --replaces "awk>=0" ../pkg_gawk
	atf_check_equal $? 0

	dulge-create -A noarch -n busybox-1.0_1 -s "busybox awk" --provides "awk-1.0_1" --replaces "awk>=0" ../pkg_busybox
	atf_check_equal $? 0

	dulge-create -A noarch -n base-system-1.1_1 -s "base-system pkg" --dependencies "awk>=0" ../pkg_base-system
	atf_check_equal $? 0
	dulge-rindex -d -a $PWD/*.dulge
	atf_check_equal $? 0
	cd ..

	dulge-install  -r root --repository=$PWD/some_repo -dyu
	atf_check_equal $? 0
...
vpkg_provider_in_conf_body() {
	...
	cd ..
	dulge-install  -r root --repository=$PWD/some_repo -dy gawk
	atf_check_equal $? 0

	mkdir -p root/dulge.d

	echo "virtualpkg=awk:gawk" > root/dulge.d/awk.conf
	out=$(dulge-query -C dulge.d -r root --repository=$PWD/some_repo -ppkgver awk)
	exp="gawk-1.0_1"
	echo "out: $out"
	echo "exp: $exp"
	atf_check_equal $out $exp

	echo "virtualpkg=awk-0_1:gawk" > root/dulge.d/awk.conf
	out=$(dulge-query -C dulge.d -r root --repository=$PWD/some_repo -ppkgver awk)
	exp="gawk-1.0_1"
	echo "out: $out"
	echo "exp: $exp"
...
vpkg_dep_provider_in_conf_body() {
	...
	cd some_repo
	dulge-create -A noarch -n blah-1.0_1 -s "blah pkg" ../pkg_blah
	atf_check_equal $? 0
	dulge-create -A noarch -n gawk-1.0_1 -s "gawk pkg" --dependencies "vpkg>=4.4" ../pkg_gawk
	atf_check_equal $? 0
	...
	mkdir -p root/dulge.d
	echo "virtualpkg=vpkg:blah" > root/dulge.d/blah.conf
	dulge-install -C dulge.d -r root --repository=$PWD/some_repo -dy gawk
	atf_check_equal $? 0

	rm -rf root
	mkdir -p root/dulge.d
	echo "virtualpkg=vpkg-4.4_1:blah" > root/dulge.d/blah.conf
	dulge-install -C dulge.d -r root --repository=$PWD/some_repo -dy gawk
	atf_check_equal $? 0
...
vpkg_incompat_upgrade_body() {
	...
	mkdir -p pkg_A pkg_B
	cd some_repo
	dulge-create -A noarch -n A-1.0_1 -s "A pkg" --provides "vpkg-9_1" ../pkg_A
	atf_check_equal $? 0
	dulge-create -A noarch -n B-1.0_1 -s "B pkg" --dependencies "vpkg-9_1" ../pkg_B
	atf_check_equal $? 0
	...
	cd some_repo
	dulge-create -A noarch -n A-2.0_1 -s "A pkg" --provides "vpkg-10_1" ../pkg_A
	atf_check_equal $? 0
	dulge-rindex -d -a $PWD/*.dulge
	atf_check_equal $? 0
	cd ..
...
vpkg_incompat_downgrade_body() {
	...
	cd some_repo
	dulge-create -A noarch -n A-0.1_1 -s "A pkg" --provides "vpkg-8_1" ../pkg_A
	atf_check_equal $? 0
	dulge-rindex -d -f -a $PWD/A-0.1_1.*.dulge
	atf_check_equal $? 0
	cd ..

	dulge-install -r root --repository=$PWD/some_repo -dyvf A-0.1_1
	# ENODEV == unresolved dependencies
	atf_check_equal $? 19
}
...
vpkg_provider_and_revdeps_downgrade_body() {
	...
	dulge-create -A noarch -n A-0.1_1 -s "A pkg" --provides "vpkg-8_1" ../pkg_A
	atf_check_equal $? 0
	dulge-create -A noarch -n B-0.1_1 -s "B pkg" --dependencies "vpkg-8_1" ../pkg_A
	atf_check_equal $? 0
	dulge-rindex -d -f -a $PWD/A-0.1_1.*.dulge $PWD/B-0.1_1.*.dulge
	atf_check_equal $? 0
	cd ..

	dulge-install -r root --repository=$PWD/some_repo -dyvf B-0.1_1 A-0.1_1
	atf_check_equal $? 0
...
vpkg_multirepo_body() {
       ...
       cd repo-1
       dulge-create -n A-1.0_1 -s A -A noarch -P V-0_1 ../empty
       atf_check_equal $? 0

       cd ../repo-2
       dulge-create -n B-1.0_1 -s B -A noarch -P V-0_1 ../empty
       atf_check_equal $? 0

       cd ..
       dulge-rindex -a repo-1/*.dulge
       atf_check_equal $? 0
       dulge-rindex -a repo-2/*.dulge
       atf_check_equal $? 0

       echo "virtualpkg=V-0_1:B" > virtualpkg.conf
       out="$(dulge-install -C $PWD -r root --repo=repo-1 --repo=repo-2 -n V|awk '{print $1}')"
