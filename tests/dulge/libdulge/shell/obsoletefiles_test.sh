...
reinstall_obsoletes_body() {
	...
	cd some_repo
	dulge-create -A noarch -n A-1.1_1 -s "foo pkg" ../pkg_A
	atf_check_equal $? 0
	dulge-rindex -d -a $PWD/*.dulge
	atf_check_equal $? 0

	cd ..
	dulge-install -r root -C null.conf --repository=$PWD/some_repo -yv A-1.1_1
	atf_check_equal $? 0

	rm -f some_repo/*
	cd some_repo
	dulge-create -A noarch -n A-1.0_1 -s "foo pkg" ../pkg_B
	atf_check_equal $? 0
	dulge-rindex -d -a $PWD/*.dulge
	atf_check_equal $? 0

	cd ..
	dulge-install -r root -C null.conf --repository=$PWD/some_repo -dyvf A-1.0_1
	atf_check_equal $? 0
...
root_symlinks_update_body() {
	...
	dulge-install -r root -C null.conf --repository=$PWD -yd foo
	atf_check_equal $? 0
	...
	dulge-install -r root -C null.conf --repository=$PWD -yuvd foo
	atf_check_equal $? 0
...
files_move_from_dependency_body() {
	...
	dulge-install -r root -C null.conf --repository=$PWD -yvd B
	atf_check_equal $? 0
	...
	dulge-install -r root -C null.conf --repository=$PWD -yuvd
	atf_check_equal $? 0

	foofile="$(dulge-query -r root -f A-1.1_1|grep foo)"
	atf_check_equal "$foofile" ""

	foofile="$(dulge-query -r root -f B-1.1_1|grep foo)"
...
files_move_to_dependency_body() {
	...
	cd repo
	dulge-create -A noarch -n libressl-1.0_1 -s "libressl pkg" ../pkg_libressl
	atf_check_equal $? 0
	dulge-create -A noarch -n libcrypto-1.0_1 -s "libcrypto pkg" --replaces "libressl<1.1_1" ../pkg_libcrypto
	atf_check_equal $? 0
	dulge-rindex -d -a $PWD/*.dulge
	atf_check_equal $? 0

	dulge-install -r root --repository=$PWD -yvd libressl
	atf_check_equal $? 0

	rm -rf ../pkg_libressl/*
	dulge-create -A noarch -n libcrypto-1.1_1 -s "libcrypto pkg" --replaces "libressl<1.1_1" ../pkg_libcrypto
	atf_check_equal $? 0
	dulge-create -A noarch -n libressl-1.1_1 -s "libressl pkg" --dependencies "libcrypto>=1.0" ../pkg_libressl
	atf_check_equal $? 0
	dulge-rindex -d -a $PWD/*.dulge
	atf_check_equal $? 0

	dulge-install -r root --repository=$PWD -yuvd
	atf_check_equal $? 0

	foofile="$(dulge-query -r root -f libressl|grep crypto)"
	atf_check_equal "$foofile" ""

	foofile="$(dulge-query -r root -f libcrypto|grep crypto)"
...
files_move_to_dependency2_body() {
	...
	dulge-create -A noarch -n libressl-1.0_1 -s "libressl pkg" ../pkg_libressl
	atf_check_equal $? 0
	dulge-create -A noarch -n libcrypto-1.0_1 -s "libcrypto pkg" ../pkg_libcrypto
	atf_check_equal $? 0
	dulge-rindex -d -a $PWD/*.dulge
	atf_check_equal $? 0
	...
	dulge-create -A noarch -n libcrypto-1.0_2 -s "libcrypto pkg" ../pkg_libcrypto
	atf_check_equal $? 0
	dulge-create -A noarch -n libressl-1.1_1 -s "libressl pkg" --dependencies "libcrypto>=1.0" ../pkg_libressl
	atf_check_equal $? 0
	dulge-rindex -d -a $PWD/*.dulge
...
update_to_meta_depends_replaces_body() {
	...
	cd repo
	dulge-create -A noarch -n openssl-1.0_1 -s "openssl pkg" ../pkg_openssl
	atf_check_equal $? 0
	dulge-rindex -d -a $PWD/*.dulge
	atf_check_equal $? 0
	cd ..

	dulge-install -r root --repository=$PWD/repo -yvd openssl
	atf_check_equal $? 0
	...
	touch -mt 197001010000.00 ../pkg_libressl/usr/lib/libcrypto.so.30
	touch -mt 197001010000.00 ../pkg_libressl/usr/lib/libssl.so.30
	dulge-create -A noarch -n openssl-1.0_2 -s "openssl pkg (meta)" --dependencies="libressl>=1.0_1" ../pkg_openssl
	atf_check_equal $? 0
	dulge-create -A noarch -n libressl-1.0_1 -s "libressl pkg" --replaces "libressl>=0" ../pkg_libressl
	atf_check_equal $? 0
	dulge-rindex -d -a $PWD/*.dulge
	atf_check_equal $? 0
	cd ..
	...
	dulge-query -r root -S openssl
	atf_check_equal $? 0

	dulge-query -r root -S libressl
	atf_check_equal $? 0
...
directory_to_symlink_body() {
	...
	dulge-install -r root -C empty.conf --repository=$PWD/some_repo -dvyu
	atf_check_equal $? 0
}
...
directory_to_symlink_preserve_body() {
	mkdir -p some_repo pkg_A/foo dulge.d
	touch pkg_A/foo/bar
	echo "preserve=/foo/bar" >dulge.d/preserve.conf
	# create package and install it
	cd some_repo
	dulge-create -A noarch -n A-1.0_1 -s "A pkg" ../pkg_A
	atf_check_equal $? 0
	...
	dulge-install -r root -C $PWD/dulge.d --repository=$PWD/some_repo -dvyu
	# ENOTEMPTY
	atf_check_equal $? 39
...
update_extract_dir_body() {
	...
	atf_check_equal $? 0
	cd ..
	dulge-install -r root -C empty.conf --repository=$PWD/some_repo -d -Suy A
	atf_check_equal $? 0
}
...
replace_package_same_files_body() {
	...
	dulge-create -A noarch -n openssl-1.0_1 -s "openssl pkg" ../pkg_openssl
	atf_check_equal $? 0
	dulge-create -A noarch -n libressl-1.0_1 -s "libressl pkg" --replaces "openssl>=0" ../pkg_libressl
	atf_check_equal $? 0
	dulge-rindex -d -a $PWD/*.dulge
	atf_check_equal $? 0
	cd ..
	...
	dulge-install -r root --repository=$PWD/repo -yvd libressl
	atf_check_equal $? 0
...
nonempty_dir_abort_body() {
	...
	dulge-install -r root --repository=$PWD/repo -yvd A B
	atf_check_equal $? 0
	...
	dulge-create -A noarch -n B-1.0_2 -s "B pkg" ../pkg_B
	atf_check_equal $? 0
	dulge-rindex -d -a $PWD/*.dulge
	atf_check_equal $? 0
	cd ..
	...
	dulge-install -r root --repository=$PWD/repo -yvdu
	# ENOTEMPTY
	atf_check_equal $? 39
	out=$(dulge-query -r root -p state A)
	atf_check_equal "$out" "installed"
	out=$(dulge-query -r root -p state B)
	atf_check_equal "$out" "installed"
	dulge-pkgdb -r root -av
	atf_check_equal $? 0
...
conflicting_files_in_transaction_body() {
	...
	dulge-install -r root -C null.conf --repository=$PWD/some_repo -ydv A B
	# EEXIST
	atf_check_equal $? 17

	# ignored file conflicts
	dulge-install -r root -C null.conf --repository=$PWD/some_repo -Iydv A B
	atf_check_equal $? 0
}
...
obsolete_dir_body() {
	...
	dulge-install -r root --repository=$PWD/repo -yvd A
	atf_check_equal $? 0
...
keep_modified_files_body() {
	...
	dulge-install -r root -C null.conf --repository=$PWD/some_repo -ydv A
	atf_check_equal $? 0
	...
	dulge-install -r root -C null.conf --repository=$PWD/some_repo -ydvu
	atf_check_equal $? 0
...
alternative_to_regular_file_body() {
	...
	cd some_repo
	dulge-create -A noarch -n A-1.1_1 -s "A pkg" \
...
replace_package_same_files_unordered_body() {
	...
	cd repo
	dulge-create -A noarch -n A-1.0_1 -s "A pkg" --dependencies "polkit-elogind>=0" ../pkg_A
	atf_check_equal $? 0
	dulge-create -A noarch -n B-1.0_1 -s "B pkg" --dependencies "A>=0" ../pkg_A
	atf_check_equal $? 0
	dulge-create -A noarch -n polkit-elogind-1.0_1 -s "polkit-elogind pkg" ../pkg_polkit-elogind
	atf_check_equal $? 0
	dulge-rindex -d -a $PWD/*.dulge
	atf_check_equal $? 0
	cd ..
	...
	dulge-query -r root -S polkit-elogind
	atf_check_equal $? 0

	dulge-query -r root -S polkit
	atf_check_equal $? 2

	cd repo
	dulge-create -A noarch -n A-1.0_2 -s "A pkg" --dependencies "polkit>=1.0_2" ../pkg_A
	atf_check_equal $? 0
	dulge-create -A noarch -n B-1.0_2 -s "B pkg" --dependencies "A>=1.0_2 polkit>=1.0_2" ../pkg_A
	atf_check_equal $? 0
	dulge-create -A noarch -n polkit-1.0_2 -s "polkit pkg" --replaces "polkit-elogind>=0" --provides "polkit-elogind-1.0_2" ../pkg_polkit
	atf_check_equal $? 0
	rm -rf ../pkg_polkit-elogind/usr
	dulge-create -A noarch -n polkit-elogind-1.0_2 -s "polkit pkg (transitional dummy package)" --dependencies "polkit>=1.0_2" ../pkg_polkit-elogind
	atf_check_equal $? 0
	dulge-rindex -d -a $PWD/*.dulge
	atf_check_equal $? 0
	cd ..

	# out=$(dulge-install -r root --repository=$PWD/repo -yvdun)
...
obsoletes_with_alternatives_body() {
	...
	dulge-create -A noarch -n A-1.0_2 -s "A pkg" --alternatives "tar:tar:/usr/bin/gtar" ../pkg_A
	atf_check_equal $? 0
	dulge-rindex -d -a $PWD/*.dulge
	atf_check_equal $? 0
	cd ..
...
multiple_obsoletes_with_alternatives_body() {
	...
	dulge-create -A noarch -n A-1.0_2 -s "A pkg" --alternatives "tar:tar:/usr/bin/gtar" ../pkg_A
	atf_check_equal $? 0
	dulge-create -A noarch -n B-1.0_2 -s "B pkg" --alternatives "tar:tar:/usr/bin/bsdtar" ../pkg_B
	atf_check_equal $? 0
	dulge-rindex -d -a $PWD/*.dulge
	atf_check_equal $? 0
	cd ..
...
multiple_obsoletes_with_alternatives_unordered_body() {
	...
	dulge-install -r root --repository=$PWD/repo -yvdu B A
...
obsolete_directory_multiple_packages1_body() {
	...
	atf_check_equal $? 0
	dulge-create -A noarch -n B-2.0_1 -s "B pkg" ../pkg_B
	atf_check_equal $? 0
	dulge-rindex -d -a $PWD/*.dulge
	atf_check_equal $? 0
	cd ..
...
obsolete_directory_multiple_packages2_body() {
	...
	dulge-install -r root --repository=$PWD/repo -yvdu A B
...
obsolete_directory_multiple_packages3_body() {
	...
	dulge-create -A noarch -n B-1.0_1 -s "B pkg" ../pkg_B
	atf_check_equal $? 0
	dulge-create -A noarch -n C-1.0_1 -s "C pkg" ../pkg_C
	atf_check_equal $? 0
	dulge-rindex -d -a $PWD/*.dulge
	atf_check_equal $? 0
	cd ..

	dulge-install -r root --repository=$PWD/repo -yvd A B C
	atf_check_equal $? 0
	...
	dulge-create -A noarch -n B-2.0_1 -s "B pkg" ../pkg_B
	atf_check_equal $? 0
	dulge-create -A noarch -n C-2.0_1 -s "C pkg" ../pkg_C
	atf_check_equal $? 0
	dulge-rindex -d -a $PWD/*.dulge
	atf_check_equal $? 0
	cd ..

	dulge-install -r root --repository=$PWD/repo -yvdu B A C
...
obsolete_directory_multiple_packages4_body() {
	...
	dulge-install -r root --repository=$PWD/repo -yvdu A B C
