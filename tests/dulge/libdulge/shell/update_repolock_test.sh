...
update_repolock_body() {
	...
	dulge-install -r root --repository=repo2 --repository=repo -yvd A-1.0_1
	atf_check_equal $? 0
	# A-1.0_1 is now locked
	dulge-pkgdb -r root -m repolock A
	atf_check_equal $? 0
	out=$(dulge-query -r root -p repository A)
	atf_check_equal "$out" "$(readlink -f repo)"
	# no update due to repository locking
	dulge-install -r root --repository=repo2 --repository=repo -yuvd
	atf_check_equal $? 0
	out=$(dulge-query -r root -p pkgver A)
	atf_check_equal $out A-1.0_1
	# disable repolock
	dulge-pkgdb -r root -m repounlock A
	atf_check_equal $? 0
	out=$(dulge-query -r root -p repolock A)
	atf_check_equal "$out" ""
...
update_repolock_explicit_body() {
	...
	atf_check_equal "$out" "$(readlink -f repo)"
	# no update due to repository locking
	dulge-install -r root --repository=repo2 --repository=repo -yuvd A
	atf_check_equal $? 0
	out=$(dulge-query -r root -p pkgver A)
	atf_check_equal $out A-1.0_1
	# disable repolock
...
reinstall_repolock_body() {
	...
	cd ..
	# install A-1.0_1 from repository "repo"
	dulge-install -r root --repository=repo -yvd A-1.0_1
	atf_check_equal $? 0
	# A-1.0_1 is now locked
	dulge-pkgdb -r root -m repolock A
	atf_check_equal $? 0
	...
	dulge-install -r root --repository=repo -yvfd A-1.0_1
	atf_check_equal $? 0
	out=$(dulge-query -r root -p pkgver A)
	atf_check_equal $out A-1.0_1
}
...
update_repolock_as_dependency_body() {
	...
	dulge-create -A noarch -n A-1.1_1 -s "A pkg" ../pkg_A
	atf_check_equal $? 0
	dulge-create -A noarch -n B-1.0_1 -s "B pkg" -D "A>=1.0_1" ../pkg_A
	atf_check_equal $? 0
	dulge-rindex -d -a $PWD/*.dulge
	atf_check_equal $? 0
	cd ..
	# install A-1.0_1 from repository "repo", B from "repo2"
	dulge-install -r root --repository=repo2 --repository=repo -yvd A-1.0_1 B
	atf_check_equal $? 0
	# A-1.0_1 is now locked
	dulge-pkgdb -r root -m repolock A
	atf_check_equal $? 0
	...
	cd repo2
	dulge-create -A noarch -n B-1.1_1 -s "B pkg" -D "A>=1.1_1" ../pkg_A
	atf_check_equal $? 0
	dulge-rindex -d -a $PWD/*.dulge
	atf_check_equal $? 0
	cd ..
