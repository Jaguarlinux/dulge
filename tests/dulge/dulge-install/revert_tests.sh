...
revert_package_head() {
	atf_set "descr" "dulge-install(1): do a full workflow of reverting a package version"
}
...
revert_package_body() {
	...
	atf_check_equal $? 0
	cd ..
	dulge-install -r root -C empty.conf --repository=$PWD/some_repo -y -u
	atf_check_equal $? 0

	# whoops, we made a mistake, rollback to 1.0_1
	cd some_repo
	echo second V1.0 > ../pkg_A/file00
	dulge-create -A noarch -n A-1.0_1 -r "1.1_1" -s "A pkg" ../pkg_A
	atf_check_equal $? 0
	dulge-rindex -d -a $PWD/*.dulge
	atf_check_equal $? 0
	cd ..
	dulge-install -d -r root -C empty.conf --repository=$PWD/some_repo -y -u
	atf_check_equal $? 0
