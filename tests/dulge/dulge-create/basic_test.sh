#! /usr/bin/env atf-sh
# Test that dulge-create(1) works as expected.

atf_test_case hardlinks_size

hardlinks_size_head() {
	atf_set "descr" "dulge-create(1): installed-size behaviour with hardlinks"
}
...
hardlinks_size_body() {
	...
	atf_check_equal $? 0
	result="$(dulge-query -r root --repository=repo -p installed_size foo)"
	expected="10B"
	rv=0
...
symlink_relative_target_head() {
	atf_set "descr" "dulge-create(1): relative symlinks in destdir must be absolute"
}
...
symlink_relative_target_body() {
	...
	dulge-rindex -d -a repo/*.dulge
	atf_check_equal $? 0
	result="$(dulge-query -r root --repository=repo -f foo|tr -d '\n')"
...
symlink_sanitize_head() {
	atf_set "descr" "dulge-create(1): symlinks must be properly sanitized"
}
...
symlink_relative_target_cwd_head() {
	atf_set "descr" "dulge-create(1): relative symlinks to cwd in destdir must be absolute"
}
...
restore_mtime_head() {
	atf_set "descr" "dulge-create(1): restore file mtime as user"
}
...
reproducible_pkg_head() {
	atf_set "descr" "dulge-create(1): generate identical packages"
}
...
reproducible_pkg_body() {
	...
	dulge-create -A noarch -n foo-1.0_1 -s "foo pkg" pkg_A
	atf_check_equal $? 0
	mv foo-1.0_1.noarch.dulge foo-1.0_1.noarch.dulge.orig
	atf_check_equal $? 0
	sleep 1
	dulge-create -A noarch -n foo-1.0_1 -s "foo pkg" pkg_A
	atf_check_equal $? 0
	cmp -s foo-1.0_1.noarch.dulge foo-1.0_1.noarch.dulge.orig
	atf_check_equal $? 0
...
reject_fifo_file_head() {
	atf_set "descr" "dulge-create(1): reject fifo file"
}
