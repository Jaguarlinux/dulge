...
EOF
	cd some_repo
	dulge-create -A noarch -n foo-1.0_1 -s "A pkg" ../pkg_A
	atf_check_equal $? 0
	dulge-rindex -d -a $PWD/*.dulge
	atf_check_equal $? 0
	cd ..
...
reverts_head() {
	atf_set "descr" "dulge-checkvers(1): test with reverts"
}
...
reverts_alpha_head() {
	atf_set "descr" "dulge-checkvers(1): test with reverts containing an alphanumeric character"
}
...
EOF
	cd some_repo
	dulge-create -A noarch -n fs-utils-1.10_1 -s "A pkg" ../pkg_A
	atf_check_equal $? 0
	dulge-rindex -d -a $PWD/*.dulge
	atf_check_equal $? 0
	cd ..
...
reverts_many_head() {
	atf_set "descr" "dulge-checkvers(1): test with multiple reverts"
}
...
EOF
	cd some_repo
	dulge-create -A noarch -n A-1.2_1 -s "A pkg" ../pkg_A
	atf_check_equal $? 0
	dulge-rindex -d -a $PWD/*.dulge
	atf_check_equal $? 0
	cd ..
	...
	dulge-rindex -c .
	atf_check_equal $? 0
	dulge-create -A noarch -n A-1.3_4 -s "A pkg" ../pkg_A
	atf_check_equal $? 0
	dulge-rindex -d -a $PWD/*.dulge
	atf_check_equal $? 0
	cd ..
...
manual_mode_head() {
	atf_set "descr" "dulge-checkvers(1): test manual mode"
}
...
removed_head() {
	atf_set "descr" "dulge-checkvers(1): test removed"
}
...
EOF
		cd some_repo
		dulge-create -A noarch -n ${pkg}-1_1 -s "$pkg pkg" .
		atf_check_equal $? 0
		cd ..
	done
...
removed_subpkgs_head() {
	atf_set "descr" "dulge-checkvers(1): handling of removed subpkgs"
}
...
EOF
		cd some_repo
		dulge-create -A noarch -n ${pkg}-1_1 -s "$pkg pkg" .
		dulge-create -A noarch -n ${pkg}-devel-1_1 -s "$pkg devel subpkg" .
		atf_check_equal $? 0
		cd ..
	done
...
multiline_reverts_head() {
	atf_set "descr" "dulge-checkvers(1): test with reverts newline characters"
}
...
EOF
	cd some_repo
	dulge-create -A noarch -n libdwarf-0.11.1_1 -s "A pkg" ../pkg_A
	atf_check_equal $? 0
	dulge-rindex -d -a $PWD/*.dulge
	atf_check_equal $? 0
	cd ..
