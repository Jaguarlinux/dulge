...
noremove_stage_body() {
	...
	cd some_repo
	atf_check -o ignore -- dulge-create -A noarch -n foo-1.0_1 -s "foo pkg" --shlib-provides "foo.so.1" ../pkg_A
	atf_check -o ignore -- dulge-create -A noarch -n bar-1.0_1 -s "foo pkg" --shlib-requires "foo.so.1" ../pkg_B
	atf_check -o ignore -- dulge-create -A noarch -n deleteme-1.0_1 -s "deleteme pkg" ../pkg_B
	atf_check -o ignore -e ignore -- dulge-rindex -a $PWD/*.dulge
	atf_check -o ignore -- dulge-create -A noarch -n deleteme-1.1_1 -s "deleteme pkg" ../pkg_B
	atf_check -o ignore -e ignore -- dulge-rindex -a $PWD/*.dulge
	atf_check -o ignore -- dulge-create -A noarch -n foo-1.1_1 -s "foo pkg" --shlib-provides "foo.so.2" ../pkg_A
	atf_check -o ignore -e ignore -- dulge-rindex -a $PWD/*.dulge
	cd ..
	atf_check -o inline:"    3 $PWD/some_repo (Staged) (RSA unsigned)\n" -- \
		dulge-query -r ../root -i --repository=some_repo -L
	atf_check -o inline:"Removed obsolete package \`deleteme-1.0_1.noarch.dulge'.\n" -- dulge-rindex -r some_repo
