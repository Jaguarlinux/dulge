#! /usr/bin/env atf-sh
# Test that dulge-query(1) list modes work as expected

atf_test_case list_repos

list_repos_head() {
	atf_set "descr" "dulge-query(1) -L"
}
...
list_repos_body() {
	...
	atf_check_equal $? 0
	dulge-create -A noarch -n baz-1.0_1 -s "baz pkg" ../pkg_A
	atf_check_equal $? 0
	dulge-rindex -d -a $PWD/*.dulge
	atf_check_equal $? 0
	rm -f *.dulge
	cd ..
	output="$(dulge-query -C empty.conf -i --repository=some_repo --repository=vanished_repo -L | tr -d '\n')"
	atf_check_equal "$output" "    2 ${PWD}/some_repo (RSA unsigned)	-1 vanished_repo (RSA maybe-signed)"

	output="$(dulge-query -C empty.conf -i --repository=https://localhost/wtf -L | tr -d '\n')"
