...
char *
dulge_pubkey2fp(dulge_data_t pubkey)
{
	...
	assert(mdctx);
	pubkeydata = dulge_data_data_nocopy(pubkey);
	bio = BIO_new_mem_buf(pubkeydata, dulge_data_size(pubkey));
	assert(bio);
	...
	if (EVP_PKEY_base_id(pPubKey) != EVP_PKEY_RSA) {
		dulge_dbg_printf("only RSA public keys are currently supported\n");
		goto out;
	}

	pRsa = EVP_PKEY_get1_RSA(pPubKey);
	if (!pRsa) {
		dulge_dbg_printf("failed to get RSA public key : %s\n",
		    ERR_error_string(ERR_get_error(), NULL));
		goto out;
	}
