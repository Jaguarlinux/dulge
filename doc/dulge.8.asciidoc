...
Name
----
dulge - package manager utility


Synopsis
--------
'dulge' <operation> [options] [targets]


Description
-----------
dulge is a package management utility that tracks installed packages on a Linux
...
automatically upgrade packages. dulge packages are a zipped tar format.

Since version 3.0.0, dulge has been the front-end to linkman:libalpm[3], the
...
Invoking dulge involves specifying an operation with any potential options and
...
*-D, \--database*::
	...
	certain attributes of the installed packages in dulge's database. It
...
*-S, \--sync*::
	...
	example, `dulge -S qt` will download and install qt and all the
	...
	package to install: `dulge -S testing/qt`. You can also specify version
	requirements: `dulge -S "bash>=3.2"`. Quotes are needed, otherwise the
	shell interprets ">" as redirection to a file.
+
In addition to packages, groups can be specified as well. For example, if
gnome is a defined package group, then `dulge -S gnome` will provide a
...
+
Packages that provide other packages are also handled. For example, `dulge -S
...
+
You can also use `dulge -Su` to upgrade all packages that are out-of-date. See
<<SO,Sync Options>> below. When upgrading, dulge performs version comparison
...
*-T, \--deptest*::
	...
	This operation accepts no other options. Example usage: `dulge -T qt
	"bash>=3.2"`.
...
*-U, \--upgrade*::
	...
	on how dulge takes care of configuration files.
...
*-b, \--dbpath* <path>::
	...
	+{localstatedir}/lib/dulge+). This should not be used unless you know what
	you are doing.
...
*-r, \--root* <path>::
	...
	*NOTE*: If database path or log file are not specified on either the
	command line or in linkman:dulge.conf[5], their default location will
...
*\--cachedir* <dir>::
	...
	+{localstatedir}/cache/dulge/pkg+). Multiple cache directories can be
	specified, and they are tried in the order they are passed to dulge.
	*NOTE*: This is an absolute path, and the root path is not automatically
	prepended. If DownloadUser is set in linkman:dulge.conf[5], then the
...
*\--gpgdir* <dir>::
	...
	(the default is +{sysconfdir}/dulge.d/gnupg+). This directory should contain
...
*\--hookdir* <dir>::
	...
	+{sysconfdir}/dulge.d/hooks+). Multiple hook directories can be specified
...
*\--noconfirm*::
	...
	this unless you want to run dulge from a script.
...
*-d, \--nodeps*::
	...
	dulge will always check a package's dependency fields to ensure that all
...
*\--noprogressbar*::
	...
	for scripts that call dulge and capture the output.
...
*\--ignore* <package>::
	Directs dulge to ignore upgrades of package even if there is one
	available. Multiple packages can be specified by separating them
	with a comma.

*\--ignoregroup* <group>::
	Directs dulge to ignore upgrades of all packages in 'group', even if
...
*-q, \--quiet*::
	...
	dulge's output is processed in a script. Search will only show package
...
*-n, \--nosave*::
	Instructs dulge to ignore file backup designations. Normally, when a
...
*-c, \--clean*::
	...
	currently unused sync databases to free up disk space. When dulge
	...
	linkman:dulge.conf[5]. Use one '\--clean' switch to only remove
...
*-u, \--sysupgrade*::
	...
	necessary.
+
Pass this option twice to enable package downgrades; in this case, dulge will
...
*-y, \--refresh*::
	...
	server(s) defined in linkman:dulge.conf[5]. This should typically be used
...
*-q, \--quiet*::
	...
	dulge's output is processed in a script, however, you may want to use
...
----------------------------
dulge uses the same logic as 'rpm' to determine action against files that are
...
dulge -U /home/user/ceofhack-0.6-1-x86_64.pkg.tar.gz::
...
Configuration
-------------
See linkman:dulge.conf[5] for more details on configuring dulge using the
'dulge.conf' file.
