...
*epoch*::
	...
	version comparison logic. See linkman:dulge[8] for more information on
	version comparisons.
...
*backup (array)*::
	...
	`"Handling Config Files"` in linkman:dulge[8] for more information.
...
*optdepends (array)*::
	...
	and are not utilized by dulge during dependency resolution. Packages in
...
+
Sysupgrade is currently the only dulge operation that utilizes this field.
...
*xdata (array)*::
	...
	This data is neither used by dulge nor by makepkg;
...
--------------------------------
dulge has the ability to store and execute a package-specific script when it
...
See Also
--------
linkman:makepkg[8], linkman:dulge[8], linkman:makepkg.conf[5]
