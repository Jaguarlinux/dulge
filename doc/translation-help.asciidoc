...
This document is here to guide you in helping translate dulge messages,
libalpm messages, and the manual pages for the entire dulge package.

We are currently using https://app.transifex.com/[Transifex] as the translation
platform for dulge and libalpm. You will need to sign up for an account there
and then register with a translation team on the
https://explore.transifex.com/toofishes/archlinux-dulge/[dulge project page].
...
There are two separate message catalogs in dulge: one for the back-end
(libalpm) and one for the front-end (dulge and scripts). These correspond to
the `lib/libalpm/po` and `po` directories in the dulge source, respectively.
...
Or to just push one of the two available resources:
	...
	tx push -r archlinux-dulge.dulge-pot -t -l <mylang>
	tx push -r archlinux-dulge.libalpm-pot -t -l <mylang>
...
and an email will be sent to the mailto:dulge-dev@lists.archlinux.org[dulge-dev]
...
dulge and dulge-scripts). Translators interested in helping are encouraged to use the
...
If you have more advanced needs you will have to get a copy of the dulge
repository.

	git clone https://gitlab.archlinux.org/dulge/dulge.git
...
the source code by running:
	...
	meson compile -C build dulge-scripts-pot dulge-pot libalpm-pot
	
We need to first update the main message catalog file. Navigate into either
the `lib/libalpm/po`, `scripts/po` or `src/dulge/po` directory depending
...
with `libalpm.pot`, `dulge.pot` or `dulge-scripts.pot`):
...
substituting `libalpm.pot` or `dulge.pot` for potfile depending on which
...
you may ignore the rest), replacing `<lang_code>` accordingly:
	...
	meson compile -C build dulge-scripts-gmo dulge-gmo libalpm-gmo
	cp ./build/lib/libalpm/po/<lang_code>/LC_MESSAGES/libalpm.mo    /usr/share/locale/<lang_code>/LC_MESSAGES/libalpm.mo
	cp ./build/scripts/po/<lang_code>/LC_MESSAGES/dulge-scripts.mo /usr/share/locale/<lang_code>/LC_MESSAGES/dulge-scripts.mo
	cp ./build/src/dulge/po/<lang_code>/LC_MESSAGES/dulge.mo      /usr/share/locale/<lang_code>/LC_MESSAGES/dulge.mo


Translating Manpages
--------------------
There are currently no efforts underway to include translated manual pages in
the dulge codebase. However, this is not to say translations are unwelcome. If
someone has experience with i18n man pages and how to best include them with our
source, please contact the dulge-dev mailing list at
mailto:dulge-dev@lists.archlinux.org[].
