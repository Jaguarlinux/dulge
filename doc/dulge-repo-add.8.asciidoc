...
packages built with linkman:makepkg[8] and installed with linkman:dulge[8].
...
dulge and a large database containing package file lists ``foo.files.tar.xz'' for
use by other utilities. While dulge can use the large database (if renamed with a
db.tar* extension), there is currently no additional benefit for the larger download.


See Also
--------
linkman:makepkg[8], linkman:dulge[8]
