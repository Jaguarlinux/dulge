...
Name
----
dulge.conf - dulge package manager configuration file


Synopsis
--------
{sysconfdir}/dulge.conf


Description
-----------
dulge, using linkman:libalpm[3], will attempt to read dulge.conf each time it
is invoked. This configuration file is divided into sections or repositories.
Each section defines a package repository that dulge can use when searching
...
*RootDir =* /path/to/root/dir::
	Set the default root directory for dulge to install to. This option is
...
*DBPath =* /path/to/db/dir::
	...
	default is +{localstatedir}/lib/dulge/+. Most users will not need to set
	this option. *NOTE*: if specified, this is an absolute path and the root
	path is not automatically prepended.

*CacheDir =* /path/to/cache/dir::
	Overrides the default location of the package cache directory. The
	default is +{localstatedir}/cache/dulge/pkg/+. Multiple cache directories can be
...
*HookDir =* /path/to/hook/dir::
	...
	+{sysconfdir}/dulge.d/hooks+.  Multiple directories can be specified with
...
*GPGDir =* /path/to/gpg/dir::
	...
	files for GnuPG. The default is +{sysconfdir}/dulge.d/gnupg/+.
...
*LogFile =* /path/to/log/file::
	Overrides the default location of the dulge log file. The default
	is +{localstatedir}/log/dulge.log+. This is an absolute path and the root directory
	is not prepended.

*HoldPkg =* package ...::
	If a user tries to '\--remove' a package that's listed in `HoldPkg`,
	dulge will ask for confirmation before proceeding. Shell-style glob
	patterns are allowed.

*IgnorePkg =* package ...::
	Instructs dulge to ignore any upgrades for this package when performing
	a '\--sysupgrade'. Shell-style glob patterns are allowed.

*IgnoreGroup =* group ...::
	Instructs dulge to ignore any upgrades for all packages in this
...
*Architecture =* auto &| i686 &| x86_64 | ...::
	If set, dulge will only allow installation of packages with the given
...
*Color*::
	Automatically enable colors only when dulge's output is on a tty.
...
*DownloadUser =* username::
	...
	option is not set then the downloads are done as the user running dulge.
...
Include = /etc/dulge.d/mirrorlist
--------
...
+
During parsing, dulge will define the `$repo` variable to the name of the
current section. This is often utilized in files specified using the 'Include'
directive so all repositories can use the same mirrorfile. dulge also defines
...
directory with these packages so dulge can find it when run with '\--refresh'.
...
in the configuration file as shown in the config example above. dulge will now
use your package repository. If you add new packages to the repository,
remember to re-generate the database and use dulge's '\--refresh' option.
...
See Also
--------
linkman:dulge[8], linkman:libalpm[3]
