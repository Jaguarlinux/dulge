...
customizations, generate meta-info, and package the whole thing up for dulge
to use.
...
*-s, \--syncdeps*::
	Install missing dependencies using dulge. When build-time or run-time
	dependencies are not found, dulge will try to resolve them. If
...
*\--noconfirm*::
	(Passed to dulge) Prevent dulge from waiting for user input before
	proceeding with operations.

*\--needed*::
	(Passed to dulge) Tell dulge not to reinstall a target if it is already
	up-to-date. (used with '-i' / '\--install').

*\--asdeps*::
	(Passed to dulge) Install packages as non-explicitly installed (used
	with '-i' / '\--install').

*\--noprogressbar*::
	(Passed to dulge) Prevent dulge from displaying a progress bar;
...
**dulge**::
	...
	install and remove packages. dulge's '-Qq', '-Rns', '-S', '-T', and '-U'
	operations must be supported by this command. If the variable is not
	set or empty, makepkg will fall back to `dulge'.
...
See Also
--------
linkman:makepkg.conf[5], linkman:PKGBUILD[5], linkman:dulge[8]
