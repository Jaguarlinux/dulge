...
static void setuseragent(void)
{
	...
	uname(&un);
	len = snprintf(agent, 100, "dulge/%s (%s %s) libalpm/%s",
...
static int parseargs(int argc, char *argv[])
{
	...
	static const struct option opts[] =
	{
		...
		{"deptest",    no_argument,       0, 'T'}, /* used by dulge-build */
...
int main(int argc, char *argv[])
{
	...
	pm_printf(ALPM_LOG_DEBUG, "dulge v%s - libalpm v%s\n", PACKAGE_VERSION, alpm_version());
	...
	switch(config->op) {
		case PM_OP_DATABASE:
			ret = dulge_database(pm_targets);
			break;
		case PM_OP_REMOVE:
			ret = dulge_remove(pm_targets);
			break;
		case PM_OP_UPGRADE:
			ret = dulge_upgrade(pm_targets);
			break;
		case PM_OP_QUERY:
			ret = dulge_query(pm_targets);
			break;
		case PM_OP_SYNC:
			ret = dulge_sync(pm_targets);
			break;
		case PM_OP_DEPTEST:
			ret = dulge_deptest(pm_targets);
			break;
		case PM_OP_FILES:
			ret = dulge_files(pm_targets);
			break;
		default:
