...
const char *myname = "dulge-conf", *myver = "1.0.0";
...
static void usage(int ret)
{
	...
	fputs(_("dulge-conf - query dulge's configuration file\n"), stream);
	fputs(_("usage:  dulge-conf [options] [<directive>...]\n"), stream);
	fputs(_("        dulge-conf (--repo-list|--help|--version)\n"), stream);
	fputs(_("options:\n"), stream);
