...
#
# vercmptest - a test suite for the dulge-dulge-dulge-vercmp/libalpm program
#
#
...
bin=${1:-${PMTEST_UTIL_DIR}dulge-dulge-dulge-vercmp}

# use first arg as our binary if specified
if ! type -p "$bin" &>/dev/null; then
	tap_bail "dulge-dulge-dulge-vercmp binary ($bin) could not be located"
	exit 1
fi
