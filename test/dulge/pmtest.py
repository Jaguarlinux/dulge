...
class pmtest(object):
    ...
    def generate(self, dulge):
        ...
        ldconfig = os.path.basename(dulge["ldconfig"])
        ldconfigdir = os.path.join(self.root, os.path.dirname(dulge["ldconfig"][1:]))
        shell = dulge["scriptlet-shell"][1:]
        ...
        for key, value in self.db.items():
            ...
            for pkg in value.pkgs:
                ...
                if self.cachepkgs:
                    pkg.dulge_build(cachedir)
                elif value.syncdir:
                    pkg.dulge_build(os.path.join(syncdir, value.treename))
                if pkg.path:
    ...
    def add_hook(self, name, content):
        if not name.endswith(".hook"):
            name = name + ".hook"
        path = os.path.join("etc/dulge.d/hooks/", name)
    ...
    def run(self, dulge):
        if os.path.isfile(util.PM_LOCK):
            tap.bail("\tERROR: another dulge session is on-going -- skipping")
            return
        ...
        prog = dulge["bin"]
        if not prog:
            prog = util.which(self.cmd[0], dulge["bindir"])
        ...
        if dulge["manual-confirm"]:
            cmd.append("--confirm")
        if dulge["debug"]:
            cmd.append("--debug=%s" % dulge["debug"])
        cmd.extend(shlex.split(self.args))

        if not (dulge["gdb"] or dulge["nolog"]):
        ...
        # Change to the tmp dir before running dulge, so that local package
        ...
        if os.path.isfile(os.path.join(self.root, util.TMPDIR, "core")):
            tap.diag("\tERROR: dulge dumped a core file")
