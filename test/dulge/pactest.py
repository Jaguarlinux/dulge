...
#
#  pactest : run automated testing on the dulge binary
#
#
...
def create_parser():
    ...
    description = "Runs automated tests on the dulge binary. Tests are " \
    ...
    parser.add_option("-d", "--debug", type = "int",
                      ...
                      help = "set debug level for dulge")
    parser.add_option("-p", "--dulge", type = "string",
                      dest = "bin", default = None,
                      help = "specify location of the dulge binary")
...
if __name__ == "__main__":
    ...
    env.dulge["debug"] = opts.debug
    env.dulge["bin"] = opts.bin
    env.dulge["bindir"] = opts.bindir
    env.dulge["nolog"] = opts.nolog
    env.dulge["gdb"] = opts.gdb
    env.dulge["valgrind"] = opts.valgrind
    env.dulge["manual-confirm"] = opts.manualconfirm
    env.dulge["scriptlet-shell"] = opts.scriptletshell
    env.dulge["ldconfig"] = opts.ldconfig
    ...
    if not opts.keeproot:
        ...
        tap.diag("dulge testing root saved: %s" % root_path)
