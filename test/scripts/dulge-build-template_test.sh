...
script=${1:-${PMTEST_SCRIPT_DIR}dulge-build-template}

if ! type -p "$script" &>/dev/null; then
	tap_bail "dulge-build-template executable (%s) could not be located" "${script}"
	exit 1
fi

TMPDIR="$(mktemp -d "/tmp/${0##*/}.XXXXXX")"
trap "rm -rf '${TMPDIR}'" EXIT TERM
cp -r "${0%/*}/dulge-build-template-tests" "$TMPDIR/dulge-build-template-tests"

# normalize paths
script="$(readlink -f $(type -p "$script"))"
cd "$TMPDIR"
testdir="./dulge-build-template-tests"
...
run_test() {
	...
	tap_diff "$TMPDIR/$testcase/result" <(printf "%s" "$expected_result") "$testcase resulting DULGEBUILD"
}
